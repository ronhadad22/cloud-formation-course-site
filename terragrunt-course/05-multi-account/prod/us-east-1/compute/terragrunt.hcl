# Compute Configuration for Production Account - us-east-1
...
inputs = {
  ...
  instance_count     = 3  # High availability in production
  ...
  user_data = <<-EOF
    ...
    echo "<h1>Production Environment - ${local.account_vars.locals.account_name}</h1>" > /var/www/html/index.html
  EOF
