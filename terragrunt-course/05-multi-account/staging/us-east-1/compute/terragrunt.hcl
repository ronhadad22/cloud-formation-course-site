# Compute Configuration for Staging Account - us-east-1
...
inputs = {
  ...
  instance_count     = 2  # More instances in staging
  ...
  user_data = <<-EOF
    ...
    echo "<h1>Staging Environment - ${local.account_vars.locals.account_name}</h1>" > /var/www/html/index.html
  EOF
